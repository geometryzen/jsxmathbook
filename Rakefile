namespace :book do
  ...
  task :build_html => 'book/contributors.txt' do
      ...
      sh "bundle exec asciidoctor #{params} -a data-uri jsxmathbook.asc"
      puts ' -- HTML output at jsxmathbook.html'
  ...
  task :build_epub => 'book/contributors.txt' do
      ...
      sh "bundle exec asciidoctor-epub3 #{params} jsxmathbook.asc"
      puts ' -- Epub output at jsxmathbook.epub'
  ...
  task :build_mobi => 'book/contributors.txt' do
      ...
      sh "bundle exec asciidoctor-epub3 #{params} -a ebook-format=kf8 jsxmathbook.asc"
      puts " -- Mobi output at jsxmathbook.mobi"
  end
  ...
  task :build_pdf => 'book/contributors.txt' do
      ...
      sh "bundle exec asciidoctor-pdf #{params} jsxmathbook.asc 2>/dev/null"
      puts ' -- PDF output at jsxmathbook.pdf'
  end
  ...
  task :check => [:build_html, :build_epub] do
      ...
      sh "htmlproofer --check-html jsxmathbook.html"
      sh "epubcheck jsxmathbook.epub"
  end
  ...
  task :clean do
    begin
        ...
        FileList['book/contributors.txt', 'jsxmathbook.html', 'jsxmathbook.epub', 'jsxmathbook.mobi', 'jsxmathbook.pdf'].each do |file|
            rm file
